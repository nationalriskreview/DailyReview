...
- **`wildfires`** — Active wildfires within 50 miles (NASA EONET). Categorized by `threat_level` (`Immediate` <15mi, `Vicinity` <50mi). Includes `acreage`, `containment_pct` (from NIFC WFIGS, joined via the fire's IRWIN id; `null` when unmatched), a `source_url` (IRWIN/InciWeb incident page), and a `sources` list. Note: evacuation orders (mandatory/voluntary) are **not** included — there is no clean national machine-readable evacuation feed; NIFC carries containment/size but not evac status.
