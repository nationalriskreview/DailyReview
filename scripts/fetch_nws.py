...
async def fetch_county_forecast_threshold(
    ...
    """Return synthetic forecast alerts if 24h precip/snow exceeds thresholds.

    Uses NWS gridpoint forecast for quantitative precipitation and snowfall.
    """
    point = await _fetch_json(
        session, NWS_POINTS_URL.format(lat=county["lat"], lon=county["lon"])
    )
    if not point:
        return []
    ...
    alerts = []
    if precip_in > PRECIP_THRESHOLD_INCHES:
        alerts.append({
            "event": "Heavy Precipitation Forecast",
            "headline": f"~{precip_in:.1f}\" rain expected in next 24h",
            "severity": "Moderate",
            "source": "nws_forecast",
        })
    if snow_in > SNOW_THRESHOLD_INCHES:
        alerts.append({
            "event": "Heavy Snow Forecast",
            "headline": f"~{snow_in:.1f}\" snow expected in next 24h",
            "severity": "Moderate",
            "source": "nws_forecast",
        })
    return alerts
